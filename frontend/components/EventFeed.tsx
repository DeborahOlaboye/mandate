...
export default function EventFeed({ events, connectionIssue }: EventFeedProps) {
  return (
        ...
        <span className="flex items-center gap-1 text-xs text-zinc-500 dark:text-zinc-400">
          <span
            className={`h-1.5 w-1.5 rounded-full ${
              connectionIssue ? "bg-amber-500" : "animate-pulse bg-emerald-500"
            }`}
          />
          {connectionIssue ? "reconnecting…" : "watching testnet"}
        </span>
      </div>
      {events.length === 0 ? (
